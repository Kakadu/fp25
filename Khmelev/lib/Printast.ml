...
let pp ?(compact = false) =
  ...
  let rec pp_impl fmt = function
    ...
    | Abs (v1, Abs (v2, t)) when compact ->
      Format.fprintf fmt "(λ %a %a -> %a)" (mangle t) v1 (mangle t) v2 pp_impl t
    | Abs (x, t) when compact -> Format.fprintf fmt "(λ %a . %a)" (mangle t) x pp_impl t
    (* Обычная лямбда (не компактный режим) *)
    | Abs (x, body) -> Format.fprintf fmt "(fun %s -> %a)" x pp_impl body
    | Const n -> Format.fprintf fmt "%d" n
    | If (c, t1, t2) ->
      Format.fprintf fmt "(if %a then %a else %a)" pp_impl c pp_impl t1 pp_impl t2
    | Let (n, e1, e2) -> Format.fprintf fmt "(let %s = %a in %a)" n pp_impl e1 pp_impl e2
    | LetRec (f, p, b, inb) ->
      Format.fprintf fmt "(let rec %s %s = %a in %a)" f p pp_impl b pp_impl inb
    | Fix t -> Format.fprintf fmt "(fix %a)" pp_impl t
    | BinOp (op, l, r) ->
      let s =
        match op with
        | Add -> "+"
        ...
        | Ge -> ">="
      in
      Format.fprintf fmt "(%a %s %a)" pp_impl l s pp_impl r
    | Prim (name, args) ->
      Format.fprintf
        fmt
        "%s(%a)"
        name
        (Format.pp_print_list ~pp_sep:Format.pp_print_space pp_impl)
        args
  in
  pp_impl
;;
