...
let rec pp_expr fmt = function
  ...
  | Abs (params, body) ->
    Format.fprintf fmt "(fun %s -> %a)" (String.concat " " params) pp_expr body
  | App (func, arg) -> Format.fprintf fmt "(%a %a)" pp_expr func pp_expr arg
  | Let (flag, name, value, body) ->
    Format.fprintf
      ...
      body
  | BinOp (op, left, right) ->
    Format.fprintf fmt "(%a %s %a)" pp_expr left (string_of_binop op) pp_expr right
  | UnOp (Neg, e) -> Format.fprintf fmt "(-%a)" pp_expr e
  | Comp (op, left, right) ->
    Format.fprintf fmt "(%a %s %a)" pp_expr left (string_of_compop op) pp_expr right
