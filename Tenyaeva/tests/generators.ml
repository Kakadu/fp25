...
let rec gen_expression n =
  ...
  then atom
    ...
    let gen_some = oneof [ return None; map (fun x -> Some x) gen_under_expr ] in
    frequency
      [ 3, atom
      ; 2, map (fun e -> Expr_option e) gen_some
      ; 2, map2 (fun t e -> Expr_constraint (t, e)) gen_type_annot gen_under_expr
      ; ( 2
        , map3
            ...
            gen_under_expr
            gen_under_expr )
      ; 2, map2 (fun op e -> Expr_unop (op, e)) gen_unary_op gen_under_expr
      ; 1, map2 (fun p e -> Expr_fun (p, e)) gen_pattern gen_under_expr
      ; 2, map2 (fun e1 e2 -> Expr_apply (e1, e2)) gen_under_expr gen_under_expr
      ; 1, map3 (fun c t e -> Expr_if (c, t, e)) gen_under_expr gen_under_expr gen_some
      ; ( 1
        , map4
            (fun rf vb vb_l e -> Expr_let (rf, vb, vb_l, e))
            gen_rec_flag
            (gen_value_binding (n / 2))
            (list_size (0 -- 2) (gen_value_binding (n / 2)))
            gen_under_expr )
      ; ( 1
        , map2
            (fun cs cs_l -> Expr_function (cs, cs_l))
            (gen_case (n / 2))
            (list_size (0 -- 2) (gen_case (n / 2))) )
      ; ( 1
        , map3
