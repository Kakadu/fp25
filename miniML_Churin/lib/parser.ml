...
let keywords = [ "let"; "rec"; "in"; "if"; "then"; "else"; "fun"; "fix" ]
...
let ident =
  ...
  then fail ("keyword '" ^ name ^ "' is reserved")
  else return name
;;
...
let parse_expression =
  let expr_parser =
    fix (fun expr ->
      ...
      in
      let unary = token (char '-') *> atom >>| (fun e -> UnOp (Negate, e)) <|> atom in
      let apply =
        unary
        >>= fun func ->
        many atom >>| fun args -> List.fold_left (fun acc arg -> App (acc, arg)) func args
      in
      let mult =
        let op =
          choice [ token (char '*') *> return Mul; token (char '/') *> return Div ]
        in
        let rec mult_parser acc =
          op
          >>= (fun op' -> apply >>= fun right -> mult_parser (BinOp (op', acc, right)))
          <|> return acc
        in
        apply >>= mult_parser
      in
      let add =
        let op =
          choice [ token (char '+') *> return Add; token (char '-') *> return Sub ]
        in
        let rec add_parser acc =
          op
          >>= (fun op' -> mult >>= fun right -> add_parser (BinOp (op', acc, right)))
          <|> return acc
        in
        mult >>= add_parser
      in
      let cmp =
        ...
        in
        op
        >>= (fun op' -> add >>= fun right -> return (CmpOp (op', left, right)))
        <|> return left
      in
      let if_expr =
      ...
      in
      let let_expr =
        token (string "let") *> (token (string "rec") *> return Recursive <|> return Plain)
        >>= fun rec_flag ->
        token ident
        >>= fun name ->
        many (token ident)
      ...
      in
      let fix_expr =
        token (string "fix") *> atom
        >>= fun f ->
        many atom
        >>| fun args -> List.fold_left (fun acc arg -> App (acc, arg)) (Fix f) args
      in
      let lambda_expr =
        token (string "fun") *> many1 (token ident)
        >>= fun params ->
        token (string "->") *> expr
        >>| fun body -> List.fold_right (fun param acc -> Lam (param, acc)) params body
      in
      choice [ let_expr; if_expr; fix_expr; lambda_expr; cmp ])
  in
