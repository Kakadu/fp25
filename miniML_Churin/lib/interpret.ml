...
module InterpM : sig
  ...
  let tick steps = if steps <= 0 then steps, Error StepLimitExceeded else steps - 1, Ok ()
...
let rec eval (env : env) (expr : expr) : value t =
    ...
    (match f_val with
     | VClosure (fname, body, closure_env) ->
       (match body with
        | Lam (param, lam_body) ->
          let rec self =
            VRecClosure (fname, param, lam_body, (fname, self) :: closure_env)
          in
          return self
  ...
  | BinOp (op, left, right) ->
    ...
    (match op with
     ...
     | Div ->
       let* a = expect_int l in
       let* b = expect_int r in
       if b = 0 then fail DivisionByZero else return (VInt (a / b)))
  | CmpOp (op, left, right) ->
    let* l = eval env left in
    let* r = eval env right in
    (match l, r with
  ...
  | If (cond, then_branch, else_branch) ->
    ...
    (match cond_val with
     | VInt _ -> if is_truthy cond_val then eval env then_branch else eval env else_branch
...
let eval_expr ?(steps = default_steps) expr = run (eval initial_env expr) steps
...
let string_of_error = function
  ...
  | InvalidCondition v ->
    Printf.sprintf "Invalid condition (expected integer): %s" (string_of_value v)
