...
let rec to_string_impl ?(parens = false) = function
  ...
  | Let (flag, name, bound, body) ->
      ...
      let body_str = to_string_impl body in
      let let_str = "let " ^ flag_to_string flag ^ name ^ " = " ^ bound_str ^ " in " ^ body_str in
      if parens then
        "(" ^ let_str ^ ")"
      else
        let_str
  ...
  | BinOp (op, left, right) ->
      let left_str = 
        if needs_parens left then
          "(" ^ to_string_impl left ^ ")"
        else
          to_string_impl left
      in
      let right_str = 
        if needs_parens right then
          "(" ^ to_string_impl right ^ ")"
        else
          to_string_impl right
      in
...
let print ?(parens = false) expr =
  print_endline (to_string ~parens expr)
