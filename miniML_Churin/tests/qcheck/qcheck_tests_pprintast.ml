...
let gen_literal =
  let open Gen in
  oneof [
    map (fun n -> Integer n) (int_range 0 20);
    return UnitVal
  ]
...
let gen_var =
  ...
  let gen_start = oneofl letters in
  let gen_rest = 
    oneof [
      return "";
      oneofl ["1"; "2"; "3"; "_acc"; "_helper";]
    ]
  in
...
let rec gen_condition depth =
  ...
  if depth <= 0 then
    ...
    frequency [
      ...
      (2, map3 (fun op l r -> CmpOp (op, l, r)) 
          (oneofl [Eq; Neq; Lt; Le; Gt; Ge]) sub sub);
...
let rec gen_expr depth : expr Gen.t =
  ...
  if depth <= 0 then
    ...
    frequency [
      ...
      (3, map3 (fun op l r -> BinOp (op, l, r)) 
          (oneofl [Add; Sub; Mul; Div]) sub sub);
      (3, map3 (fun op l r -> CmpOp (op, l, r)) 
          (oneofl [Eq; Neq; Lt; Le; Gt; Ge]) sub sub);
      (3, map2 (fun f a -> App (f, a)) sub small_sub);
      (2, map2 (fun p b -> Lam (p, b)) gen_var small_sub);
      (2, map3 (fun c t e -> If (c, t, e)) 
          (gen_condition 2)  
          small_sub small_sub);
      (2, map4 (fun flag n b body -> Let (flag, n, b, body))
          (oneofl [Plain; Recursive]) gen_var small_sub small_sub);
    ]

let arb_expr : expr arbitrary =
  make ~print:to_string (gen_expr 5)
...
let test_print_parse_identity =
  Test.make
    ~name:"Pprint.to_string >> Parser.parse = identity"
    arb_expr
    (fun expr ->
...
let test_print_idempotent =
  Test.make
    ~name:"Pprint.to_string is idempotent"
    arb_expr
    (fun expr ->
...
let () =
  QCheck_base_runner.run_tests_main [
    test_print_parse_identity;
    test_print_idempotent;
