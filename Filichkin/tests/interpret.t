...
in the dune file
  ...
  $ ../bin/REPL.exe << EOF
  > let fix f = (fun x -> (f fun v -> ((x x) v)) fun x -> (f fun v -> ((x x) v))) in let fact = (fix fun f n -> (if (n > 0) then ((f (n-1)) * n) else 1)) in (println_int (fact 5))
  > EOF
  Let (NonRec, "Var fix", Abs ("f", App (Abs ("x", App (Var "f", Abs ("v", App (App (Var "x", Var "x"), Var "v")))), Abs ("x", App (Var "f", Abs ("v", App (App (Var "x", Var "x"), Var "v")))))), Some Let (NonRec, "Var fact", App (Var "fix", Abs ("f", Abs ("n", If (BinOp (More, Var "n", Int 0), BinOp (Mult, App (Var "f", BinOp (Minus, Var "n", Int 1)), Var "n"), Int 1)))), Some App (Var "println_int", App (Var "fact", Int 5))))
  Type: unit
  120
  ()
  
  $ ../bin/REPL.exe << EOF
  > let fact n = let rec helper n k = (if (n > 0) then ((helper (n-1)) fun r -> (k (n * r))) else (k 1)) in ((helper n) fun n -> n) in (println_int (fact 10))
  > EOF
  Let (NonRec, "Var fact", Abs ("n", Let (Rec, "Var helper", Abs ("n", Abs ("k", If (BinOp (More, Var "n", Int 0), App (App (Var "helper", BinOp (Minus, Var "n", Int 1)), Abs ("r", App (Var "k", BinOp (Mult, Var "n", Var "r")))), App (Var "k", Int 1)))), Some App (App (Var "helper", Var "n"), Abs ("n", Var "n")))), Some App (Var "println_int", App (Var "fact", Int 10)))
  Type: unit
  3628800
  ()
  
  $ cat > ocaml_pow.ml <<'EOF'
  > let rec pow x n = if n = 0 then 1 else x * pow x (n - 1)
  > let () = Printf.printf "%d\n" (pow 2 5)
  > EOF
  $ ocamlc ocaml_pow.ml -o ocaml_pow.exe
  $ ./ocaml_pow.exe
  32
  $ cat > ocaml_adt.ml <<'EOF'
  ...
  > let () = Printf.printf "%d\n" (to_int Green)
  > EOF
  $ ocamlc ocaml_adt.ml -o ocaml_adt.exe
  $ ./ocaml_adt.exe
  1
