...
let rec eval (env : env) (e : expr) : value eval_result =
  match e with
  | Int n -> return (VInt n)
  | Var x ->
  ...
  | BinOp (op, e1, e2) ->
    eval env e1 >>= fun v1 -> eval env e2 >>= fun v2 -> eval_binop op v1 v2
  ...
  | App (fun_expr, arg_expr) ->
    let* vf = eval env fun_expr in
    let* va = eval env arg_expr in
    (match vf with
...
let run_interpret expr = eval Env.empty expr
