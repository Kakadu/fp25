...
let gen_expr =
  ...
  let rec expr depth =
    if depth <= 0 then
      oneof [ map (fun i -> Int i) (int_range (-100) 100)
            ; map (fun s -> Var s) gen_name ]
    else
      let new_depth = depth - 1 in
      frequency
        [ (3, map (fun i -> Int i) (int_range (-100) 100))
        ; (3, map (fun s -> Var s) gen_name)
        ; (1, map3 (fun op l r -> Binop (op, l, r)) gen_binop (expr new_depth) (expr new_depth))
        ; (1, map3 (fun op l r -> Cmp (op, l, r)) gen_cmpop (expr new_depth) (expr new_depth))
        ; (1, map2 (fun name body -> Abs (name, body)) gen_name (expr new_depth))
        ; (2, map2 (fun e1 e2 -> App (e1, e2)) (expr new_depth) (expr new_depth))
        ; (1, map3 (fun c t e -> If (c, t, e)) (expr new_depth) (expr new_depth) (expr new_depth))
        ; (1, map3 (fun x e1 e2 -> Let (x, e1, e2)) gen_name (expr new_depth) (expr new_depth))
        ; (1, map3 (fun x e1 e2 -> Let_rec (x, e1, e2)) gen_name (expr new_depth) (expr new_depth))
        ]
  in
...
let test_print_parse_roundtrip =
  QCheck.Test.make ~count:100 ~name:"Roundtrip: Parse(Print(e)) == e" arbitrary_expr
    (fun ast ->
...
let test_printer_safety =
  QCheck.Test.make ~count:100 ~name:"Printer Safety" arbitrary_expr
    (fun ast ->
      try let _ = print_expr ast in true
      with _ -> false)
...
let test_parser_negative =
  ...
  QCheck.Test.make ~count:(List.length invalid_inputs) ~name:"Negative Parser Tests" 
