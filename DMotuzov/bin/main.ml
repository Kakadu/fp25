...
type config = { filename : string option }

let default_config = { filename = None }
...
let rec parse_args args config =
  ...
  | arg :: rest ->
    ...
    then parse_args rest { filename = Some arg }
...
let () =
  ...
  | Ok { filename = Some filename } ->
    ...
    (match parse source_code with
     ...
     | Ok program ->
       (match run_interpreter program with
        | Ok _env -> ()
        | Error err -> Printf.eprintf "%s\n" (show_runtime_error err)))
  | Ok { filename = None } -> Printf.eprintf "Usage: %s <filename>\n" Sys.argv.(0)
