...
let rec mlterm_to_string : mlterm -> string = function
    ...
    | Bool false -> "false"
    | Unit -> "()"
    | ITE (c,th,e) -> "(if " ^ (mlterm_to_string c) ^ " then " ^ (mlterm_to_string th) ^ " else " ^ (mlterm_to_string e) ^ ")"
    | Let (v,t1,t2) -> "let " ^ v ^ "=" ^ (mlterm_to_string t1) ^ " in " ^ (mlterm_to_string t2)
    | LetRec (v,t1,t2) -> "let rec " ^ v ^ "=" ^ (mlterm_to_string t1) ^ " in " ^ (mlterm_to_string t2)
...
let rec compile_to_lambda_cbv : mlterm -> lterm = function
    ...
    | LetRec (v,t1,t2) -> App (Abs (v, compile_to_lambda_cbv t2), App (fix_comb_cbv, Abs (v, compile_to_lambda_cbv t1)))
