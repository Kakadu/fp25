...
let fix_comb_no =
    let helper = Abs ("x", App (Var "f", App (Var "x", Var "x")))
    in Abs ("f", App (helper, helper))
let fix_comb_cbv =
    let helper = Abs ("x", App (Var "f", Abs ("v", App (App (Var "x", Var "x"), Var "v"))))
    in Abs ("f", App (helper, helper))
let fix_comb_cbn = fix_comb_no

let try_stdlib = function
    | App (Var "println_int", Int i) -> let () = Printf.printf "%d\n%!" i in Some Unit
...
let fresh_var vars = 
    let cands = List.init (List.length vars + 1) (fun i -> "fv" ^ (string_of_int i))
    in let res = List.filter (fun x -> not (List.mem x vars)) cands
    in match res with
        | h::_ -> h
...
let vars t =
    let rec helper = function
        ...
        | _ -> []
    in List.sort_uniq compare (helper t)
...
let rec rename_free_var term x y = match term with
...
(* term[x |-> t] *)
let subst term x t =
    let rec helper term x t vars = match term with
        ...
        | Abs (y, t2) -> match List.find_opt ((=) y) (fvars t) with
            | Some _ -> let u = fresh_var vars in
                let r = helper (rename_free_var t2 y u) x t ([u] @ vars) in
                    (Abs (u, fst r), snd r)
            | None -> let r = helper t2 x t vars in
                (Abs (y, fst r), snd r)
    in fst (helper term x t ([x] @ (vars term) @ (vars t)))

let rec beta_step_in_cbv term = match try_stdlib term with
    | Some t -> Some t
    | None -> (match term with
        | Abs (_, _) -> None
        | App (Abs (x, y), z) -> (match beta_step_in_cbv z with
            | Some t -> Some (App (Abs (x, y), t))
            | None -> Some (subst y x z))
        | App (t1, t2) -> (match beta_step_in_cbv t1 with
            | Some t -> Some (App (t, t2))
            | None -> (match beta_step_in_cbv t2 with
                | Some t -> Some (App (t1, t))
                | None -> None))
        | _ -> None)

let rec beta_reduce strategy x = (* let () = Printf.printf "%s\n" (lterm_to_string x) in *)
