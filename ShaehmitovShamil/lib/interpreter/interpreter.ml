...
module Interpreter: Eval = struct
  ...
  type value =
    | VInt of int
    | VUnit
    | VClosure of env * pattern list * expr
...
;;
  ...
  let rec eval_expr (env: env) (e: expr) : value eval_result =
    ...
    | FunExpr (params, body) -> return (VClosure (env, params, body))
    | Let (NonRec, p, e1, e2) ->
      eval_expr env e1
      >>= (fun v1 ->
        match p with
        | PVar name ->
          let new_env = (name, v1) :: env in
          eval_expr new_env e2
        | PAny -> eval_expr env e2)
    | Let (Rec, p, e1, e2) ->
       (match p, e1 with
       | PVar name, FunExpr (params, body) ->
         (* Создаем рекурсивное окружение для функции *)
         let rec new_env = (name, VClosure (new_env, params, body)) :: env in
         eval_expr new_env e2
          ...
          | VClosure (closure_env, params, body) ->
            (match params with
             ...
             | p :: rest ->
               let new_env =
                 (match p with
                  | PVar name -> (name, arg_val) :: closure_env
                  | PAny -> closure_env)
               in
               if List.length rest = 0
               then eval_expr new_env body
               else return (VClosure (new_env, rest, body)))
          | VBuiltin builtin_fn -> builtin_fn arg_val
          | _ -> Error (TypeError "Cannot apply a non-function")))
  ;;

  let eval_program_item (env : env) (item : structure_item) : (env * value option) eval_result =
    match item with
    ...
    | Value (NonRec, p, expr) ->
      ...
      >>= (fun v ->
        match p with
        | PVar name -> return ((name, v) :: env, None)
