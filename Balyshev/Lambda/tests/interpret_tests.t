...
in the dune file
  ...
  $ ../bin/LambdaREPL.exe -cbv -dparsetree <<EOF
  > \f.x
  Parsed result: (Abs (f, (Var x)))
  Evaluated!
  Result: (λ f . x)

  $ ../bin/LambdaREPL.exe -dparsetree <<EOF
  > garbage242
  Error: : end_of_input



  $ ../bin/LambdaREPL.exe -no -dparsetree <<EOF
...
Below we redirect contents of the file to the evaluator
  $ ../bin/LambdaREPL.exe -dparsetree -stop-after parsing   < lam_1+1.txt
  Parsed result: (App (
                    (Abs (m,
                       (Abs (n,
                          (Abs (f,
  ...
  $ ../bin/LambdaREPL.exe -ao   < lam_1+1.txt
  Evaluated!
  Result: (λ n . (λ f . (λ x . (λ _x . ((f (n (f x))) _x)))))
  $ ../bin/LambdaREPL.exe -ao   < lam_2x1.txt
  Evaluated!
  Result: (λ z . (λ x . (z (z x))))
Call by value doesn't reduce under abstraction
  $ ../bin/LambdaREPL.exe -cbv   < lam_2x1.txt
  Evaluated!
  Result: (λ z . ((λ f . (λ x . (f (f x)))) ((λ f . (λ x . (f x))) z)))
  $ ../bin/LambdaREPL.exe -ao -small   < lam_3x2.txt
  ...
  $ ../bin/LambdaREPL.exe -ao   < lam_zero.txt
  Evaluated!
  Result: (λ g . (λ y . y))
For 3! we use noral order reduction
  ...
  $ ../bin/LambdaREPL.exe -no   < lam_fac3.txt
  Evaluated!
  Result: (λ z . (λ x . (z (z (z (z (z (z x))))))))

tests for limited reductions
  $ ../bin/LambdaREPL.exe -cbn -lim 1 <<EOF
  > \x. (\y.y) z
  Evaluated! Reductions left: 1.
  Result: (λ x . ((λ y . y) z))

  $ ../bin/LambdaREPL.exe -cbv -lim 1 <<EOF
  > \x. (\y.y) z
  Evaluated! Reductions left: 1.
  Result: (λ x . ((λ y . y) z))

  $ ../bin/LambdaREPL.exe -ao -lim 1 <<EOF
  > \x. (\y.y) z
  Evaluated! Reductions left: 0.
  Result: (λ x . z)

  $ ../bin/LambdaREPL.exe -no -lim 1 <<EOF
  > \x. (\y.y) z
  Evaluated! Reductions left: 0.
  Result: (λ x . z)
...
various strategies of frst which ignores Omega combinator
  $ ../bin/LambdaREPL.exe -cbn -lim 10 <<EOF
  > (\x.\y.x) a ((\x.x x) (\y.y y))
  Evaluated! Reductions left: 8.
  Result: a

  $ ../bin/LambdaREPL.exe -cbv -lim 10 <<EOF
  > (\x.\y.x) a ((\x.x x) (\y.y y))
  Partial evaluated.
  Result: ((λ y . a) ((λ y . (y y)) (λ y . (y y))))

  $ ../bin/LambdaREPL.exe -ao -lim 10 <<EOF
  > (\x.\y.x) a ((\x.x x) (\y.y y))
  Partial evaluated.
  Result: ((λ y . a) ((λ y . (y y)) (λ y . (y y))))

  $ ../bin/LambdaREPL.exe -no -lim 10 <<EOF
  > (\x.\y.x) a ((\x.x x) (\y.y y))
  Evaluated! Reductions left: 8.
  Result: a

lam_*.txt
  $ ../bin/LambdaREPL.exe -cbn   < lam_2x1.txt
  Evaluated!
  Result: (λ z . ((λ f . (λ x . (f (f x)))) ((λ f . (λ x . (f x))) z)))
  ...
  $ ../bin/LambdaREPL.exe -no   < lam_2x1.txt
  Evaluated!
  Result: (λ z . (λ x . (z (z x))))

  $ ../bin/LambdaREPL.exe -cbn   < lam_3x2.txt
  Evaluated!
  Result: (λ z . ((λ f . (λ x . (f (f (f x))))) ((λ f . (λ x . (f (f x)))) z)))

  $ ../bin/LambdaREPL.exe -cbv   < lam_3x2.txt
  Evaluated!
  Result: (λ z . ((λ f . (λ x . (f (f (f x))))) ((λ f . (λ x . (f (f x)))) z)))

  $ ../bin/LambdaREPL.exe -ao   < lam_3x2.txt
  Evaluated!
  Result: (λ z . (λ x . (z (z (z (z (z (z x))))))))

  $ ../bin/LambdaREPL.exe -no   < lam_3x2.txt
  Evaluated!
  Result: (λ z . (λ x . (z (z (z (z (z (z x))))))))

  $ ../bin/LambdaREPL.exe -no -lim 200 < lam_fac3.txt
  Evaluated! Reductions left: 63.
  Result: (λ z . (λ x . (z (z (z (z (z (z x))))))))

  $ ../bin/LambdaREPL.exe -ao -lim 1000 < lam_fac3.txt
  Partial evaluated.
  ...
  $ ../bin/LambdaREPL.exe -no -lim 2000 < lam_fac6.txt
...
small step evaluation
  ...
  $ ../bin/LambdaREPL.exe -cbn -small <<EOF
  > (λx.x) ((λy.y) (λz.z))
   -- ((λ y . y) (λ z . z))
   -- (λ z . z)
  Evaluated!
  ...
  $ ../bin/LambdaREPL.exe -cbv -small <<EOF
  > (λx.x) ((λy.y) (λz.z))
   -- ((λ x . x) (λ z . z))
   -- (λ z . z)
  Evaluated!
  ...
  $ ../bin/LambdaREPL.exe -ao -small <<EOF
  > (λx.(λy.y) x)
   -- (λ x . x)
  Evaluated!
  Result: (λ x . x)
  ...
  $ ../bin/LambdaREPL.exe -no -small < lam_3x2.txt
   -- ((λ y z -> ((λ f x -> (f (f (f x)))) (y z))) 2)
  ...
  $ ../bin/LambdaREPL.exe -ao -small < lam_3x6.txt
