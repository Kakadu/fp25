...
let gen_const =
  Gen.frequency
    [ 4, Gen.map (fun n -> Int n) Gen.small_int
    ; 1, Gen.return Unit
    ]
...
let rec gen_expr depth =
  let leaf =
    [ Gen.map (fun c -> Ast.Const c) gen_const
    ; Gen.map (fun n -> Ast.Var n) gen_name
    ]
  in
  if depth <= 0 then Gen.oneof leaf
  else
    let sub = gen_expr (depth / 2) in
    let simpler = gen_expr (depth - 1) in
    let let_gen =
      let open Gen in
      oneofl [ NonRec; Rec ] >>= fun kind ->
      gen_name >>= fun name ->
      (match kind with
       | Rec -> map2 (fun arg body -> Ast.Fun (arg, body)) gen_name simpler
       | NonRec -> simpler)
      >>= fun rhs ->
      option sub >|= fun body_opt ->
      let scope = match body_opt with None -> GlobalVar | Some _ -> LocalVar in
      Ast.Let (scope, kind, name, rhs, body_opt)
    in
    Gen.oneof
      (leaf
...
let arb_expr =
  make
    ...
    (Gen.sized (fun sz -> gen_expr (1 + (sz mod 6))))
...
let prop_stable_string =
  Test.make ~name:"pretty-print is idempotent after parse" ~count:500 arb_expr
    (fun e ->
