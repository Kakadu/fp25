...
let expect_unit ?steps s =
  ...
  | Ok v ->
    (failwith (Printf.sprintf "expected unit, got %s" (Interpret.string_of_value v))
      [@coverage off])
  | Error err -> (failwith (Interpret.string_of_error err) [@coverage off])
;;
...
let expect_error ?steps s expected_msg =
  ...
  | Ok v ->
    (failwith
       (Printf.sprintf "expected error %s, got value %s" expected_msg
          (Interpret.string_of_value v))
      [@coverage off])
  | Error err ->
    let got = Interpret.string_of_error err in
    if not (String.equal got expected_msg)
    then (failwith (Printf.sprintf "expected error %s, got %s" expected_msg got) [@coverage off])
;;
...
let () =
  ...
  (match Interpret.run (parse "let x = 4") with
   ...
   | Ok v ->
     (failwith (Printf.sprintf "expected 4, got %s" (Interpret.string_of_value v))
       [@coverage off])
  ...
  (match Interpret.run (parse "let rec f = fun x -> x") with
   ...
   | Ok v ->
     (failwith (Printf.sprintf "expected closure, got %s" (Interpret.string_of_value v))
       [@coverage off])
   | Error e -> (failwith (Interpret.string_of_error e) [@coverage off]));
  ...
  expect_int
    "fix (fun self -> fun n -> if n = 0 then 1 else n * self (n - 1)) 5"
    120;
