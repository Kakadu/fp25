...
let* _ = symbol "let" in
let* is_rec =
   (symbol "rec" *> return true) <|> return false
   in
   (** let rec*)
   let* name = ident in
   if is_rec then
   ...
   let lam = List.fold_right (fun p acc -> Ast.Abs (p, acc)) params body in return lam
   (** =, <, > *)
   and cmp () : Ast.name Ast.t t = 
     let* left = add_sub () in
     let parse_op = (symbol "=" *> return Ast.Eq) <|> (symbol "<" *> return Ast.Lt)
     <|> (symbol ">" *> return Ast.Gt) in
     (let* op = parse_op in
      ...
      and add_sub () : Ast.name Ast.t t =
         let op_add = 
              symbol "+" *> return (fun e1 e2 -> Ast.Binop (Ast.Add, e1, e2))
            in
            let op_sub =
               symbol "-" *> return (fun e1 e2 -> Ast.Binop (Ast.Sub, e1, e2))
            in
            chainl1 (mul_div ()) (op_add <|> op_sub)
      (** *, / *)
      and mul_div () : Ast.name Ast.t t =
        let op_mul = symbol "*" *> return (fun e1 e2 -> Ast.Binop (Ast.Mul, e1, e2))
        in
        let op_div = symbol "/" *> return (fun e1 e2 -> Ast.Binop (Ast.Div, e1, e2))
        in
...
let parse_lam : dispatch =
    let single (_pack : dispatch) : Ast.name Ast.t Angstrom.t =
        expr ()
    in
    let apps (_pack : dispatch) : Ast.name Ast.t Angstrom.t = 
        expr ()
    in
    {single; apps }
