...
let rec pp fmt = function
  ...
  | If (cond, t, e) -> Format.fprintf fmt "(if %a then %a else %a)" pp cond pp t pp e
  | App (func, arg) -> Format.fprintf fmt "(%a %a)" pp func pp arg
  | Fun (arg_name, body) -> Format.fprintf fmt "(fun %s -> %a)" arg_name pp body
