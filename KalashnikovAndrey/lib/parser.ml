...
let is_keyword_next_helper input key = 
  ...
  if not (String.starts_with ~prefix:key input) then false
  else
    if String.length input == k_len then true
...
and parse_compare () = chainl1 (parse_arithmetic()) (parse_eq_op <|> parse_lt_op <|> parse_mt_op) 
and parse_app () = 
  let* head = parse_factor() in 
  let rec loop acc =
    let* is_kwd = is_keyword_next in
    if is_kwd then return acc
    else
      (let* arg = parse_atom() in loop (App(acc, arg)))
      <|> return acc 
  in
  loop head
...
let parse str =
  match
    parse_string ~consume:Consume.All (parse_expr ()) str
  with
  | Ok x -> Ok x
