...
let pp_error ppf (err : error) =
  ...
  | Type_error s -> Format.fprintf ppf "Type error: %s" s
;;
...
module StateError = struct
  ...
  let step steps = if steps <= 0 then Error Out_of_steps, steps else Ok (), steps - 1
...
let rec check_env key : env -> value StateError.t = function
  ...
  | (k, v) :: _ when k = key -> StateError.return v
  | _ :: rest -> check_env key rest
;;
...
let run_eval input step printer_flag =
  ...
  | Error err -> Format.printf "Parse error: %a\n%!" Parser.pp_error err
  | Ok ast ->
    (match eval initial_env ast step with
     | Ok value, steps when printer_flag ->
       Format.printf "Ast: %a\n%!" pp ast;
       Format.printf "Value: %a\nSteps: %d\n%!" pp_value value steps
     | Ok value, steps -> Format.printf "Value: %a\nSteps: %d\n%!" pp_value value steps
     | Error err, steps -> Format.printf "Error: %a\nSteps: %d\n%!" pp_error err steps)
;;
