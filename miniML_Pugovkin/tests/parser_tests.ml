...
let pp_expr s =
  try Parser.expr_of_string s |> Ast.show_expr |> print_endline
  with Parser.Error msg -> Printf.printf "Error: %s\n" msg

let pp_top s =
  try Parser.toplevel_of_string s |> Ast.show_toplevel |> print_endline
  with Parser.Error msg -> Printf.printf "Error: %s\n" msg

let pp_prog s =
  try Parser.program_of_string s |> Ast.show_program |> print_endline
  with Parser.Error msg -> Printf.printf "Error: %s\n" msg
...
let%expect_test "arithmetic binops" =
  ...
  pp_expr "8 / 2 / 2";
  [%expect {|
    BinopArithmetic (Add, Const (Int 1), BinopArithmetic (Mul, Const (Int 2), Const (Int 3)))
    BinopArithmetic (Mul, BinopArithmetic (Add, Const (Int 1), Const (Int 2)), Const (Int 3))
    BinopArithmetic (Sub, BinopArithmetic (Sub, Const (Int 4), Const (Int 5)), Const (Int 6))
    BinopArithmetic (Div, BinopArithmetic (Div, Const (Int 8), Const (Int 2)), Const (Int 2))
  |}]
...
let%expect_test "toplevel items" =
  ...
  [%expect {|
    ExprTop (BinopArithmetic (Add, Var "x", Const (Int 1)))
    LetTop (Nonrec, "x", Const (Int 1))
    LetTop (Rec, "f", Lam ("x", Var "x"))
  |}]

let%expect_test "program" =
  pp_prog "let x = 1;; x + 2";
  [%expect {|
    [LetTop (Nonrec, "x", Const (Int 1)); ExprTop (BinopArithmetic (Add, Var "x", Const (Int 2)))]
  |}]
