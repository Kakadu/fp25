...
let oneof_list_gen xs =
  ...
  | _ -> int_bound (List.length xs - 1) >>= fun i -> return (List.nth xs i)
;;
...
let gen_ident =
  oneof_list_gen
    [ "x"; "y"; "z"; "f"; "g"; "h"; "n"; "m"; "k"; "foo"; "bar"; "_x"; "x1"; "y'" ]
;;
...
let gen_const =
  let open Gen in
  oneof_weighted_gen
    [ 6, map (fun n -> Ast.Int n) (int_bound 1000); 1, return (Ast.Unit ()) ]
;;
...
let rec gen_expr size =
  ...
  then atom
    ...
    oneof_weighted_gen
      ...
      ; ( 2
        , map4
            (fun rf name rhs body -> Ast.Let (rf, name, rhs, body))
            gen_rec_flag
            gen_ident
            next
            next )
...
let gen_toplevel size =
  ...
  oneof_weighted_gen
    ...
    ; ( 2
      , map3
          (fun rf name rhs -> Ast.TLet (rf, name, rhs))
          gen_rec_flag
          gen_ident
          (gen_expr size) )
    ]
...
let roundtrip_toplevel =
  Test.make
    ~name:"parser/printer roundtrip (toplevel)"
    ~count:200
    arb_toplevel
    (fun item ->
...
let roundtrip_program =
  Test.make
    ~name:"parser/printer roundtrip (program)"
    ~count:200
    arb_program
    (fun program ->
