...
let spaces : unit Angstrom.t =
  let ws1 = skip_while1 is_space *> Angstrom.return () <|> comment in
  Angstrom.fix (fun loop -> ws1 *> loop <|> Angstrom.return ())
;;
...
let reserved =
  [ "if"; "then"; "else"; "let"; "rec"; "in"; "fun"; "fix"; "not"; "true"; "false" ]
;;
...
let p_bool : expr Angstrom.t =
  choice
    [ kw "true" *> return (Const (Bool true)); kw "false" *> return (Const (Bool false)) ]
;;
...
let p_expr : expr Angstrom.t =
  fix (fun expr ->
    ...
    let p_if : expr Angstrom.t =
      ...
      *>
      let* t = expr in
      kw "else" *> (expr >>| fun e -> If (c, t, Some e)) <|> return (If (c, t, None))
    in
    let p_fun : expr Angstrom.t =
      kw "fun"
      *>
      let* params =
        many1 p_ident <|> fail "Expected at least one parameter after 'fun'"
      in
      sym "->"
      *>
      let+ body = expr in
...
let expr_of_string s = run_exn (spaces *> p_expr <* option () sep <* end_of_input) s
let toplevel_of_string s = run_exn (spaces *> p_toplevel <* many sep <* end_of_input) s
let program_of_string s = run_exn p_program s
