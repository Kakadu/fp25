...
module STATEERROR = struct
  ...
  let ( >>= ) : ('st, 'a) t -> ('a -> ('st, 'b) t) -> ('st, 'b) t =
   fun x f st ->
    let st, x = x st in
    match x with Result.Ok x -> f x st | Result.Error e -> fail e st

  let ( *> ) : ('st, 'a) t -> ('st, 'b) t -> ('st, 'b) t =
   fun x1 x2 -> x1 >>= fun _ -> x2

  let ( <|> ) : ('st, 'a) t -> ('st, 'a) t -> ('st, 'a) t =
   fun x1 x2 st ->
    let st, x = x1 st in
    match x with Result.Ok x -> return x st | Result.Error _ -> x2 st

  let ( >>| ) : ('st, 'a) t -> ('a -> 'b) -> ('st, 'b) t =
   fun x f st ->
    let st, x = x st in
    match x with Result.Ok x -> return (f x) st | Result.Error e -> fail e st

  let lift2 : ('a -> 'b -> 'c) -> ('st, 'a) t -> ('st, 'b) t -> ('st, 'c) t =
   fun f a b ->
    a >>= fun r_a ->
    b >>= fun r_b -> return @@ f r_a r_b
...
module TYPECHECK = struct
  ...
  let read_local : 'a IdMap.t t = read >>= function _, l, _, _, _ -> return l
  ...
  let read_meth_type : _type option t =
    read >>= function _, _, _, m_t, _ -> return m_t

  let read_main_class : class_with_main option t =
    read >>= function _, _, _, _, main -> return main

  let write_main_class main =
    read >>= function g, l, n, t, _ -> write (g, l, n, t, main)

  let write_global n_g =
    read >>= function _, l, n, m, main -> write (n_g, l, n, m, main)
  ...
  let get_curr_class_name : curr_class t =
    ...
    | _ ->
        fail
          (TCError
             (ImpossibleResult "Current class can be 'none' only before running"))

  let write_curr_class_name n =
    read >>= function g, l, _, t, main -> write (g, l, Some n, t, main)
end
