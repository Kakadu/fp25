...
let vartype_to_type = function TypeVar t -> t
let vardecl_to_type = function Var (t, _) -> return (vartype_to_type t)
let name_to_obj_ctx n = read_local_el n

let eq f e1 e2 =
  match f e1 e2 with true -> return e1 | false -> fail (TCError TypeMismatch)

let eq_type t1 t2 = eq equal__type t1 t2
let eq_ident n1 n2 = eq equal_ident n1 n2

let eq_ident_return_ctx n1 n2 m f =
  match equal_ident n1 n2 with true -> Some (f m) | false -> None
...
let get_class_name = function Class (_, id, _) -> id
...
let find_memb_from_obj obj_id id =
  ...
  read_global_el obj_id >>= function
...
let is_public obj_id ctx mds =
  ...
  <|> (read_global_el obj_id >>= fun _ -> fail (TCError AccessError))

let find_obj_memb_with_fail n_obj n_mem =
  find_memb_from_obj n_obj n_mem >>= function
  | Some memb -> (
      match memb with
...
let typecheck_method_args (Params params) (Args args) expr_tc =
  let params_to_list_of_type p =
    List.map (function Var (t, _) -> vartype_to_type t) p
  in
  let args_to_list_of_type a =
    map (fun x -> expr_tc x >>= fun x -> find_memb_type x) a
  in
  ...
  in
  args_to_list_of_type args >>= fun args ->
...
let typecheck_bin_op b e1 e2 expr_tc =
  let compare_two_expr_type e1 e2 =
    find_expr_type e1 expr_tc >>= fun e1 ->
    find_expr_type e2 expr_tc >>= fun e2 -> eq_type e1 e2
  in
  ...
  | OpAdd | OpMul | OpSub | OpDiv | OpMod ->
      compare_three_expr_type e1 e2 (TypeBase TypeInt)
      *> return_rez (TypeBase TypeInt)
  | OpLess | OpLessEqual | OpMore | OpMoreEqual ->
      compare_three_expr_type e1 e2 (TypeBase TypeInt)
      *> return_rez (TypeBase TypeBool)
  | OpEqual | OpNonEqual ->
      compare_two_expr_type e1 e2 *> return_rez (TypeBase TypeBool)
  | OpAnd | OpOr ->
      compare_three_expr_type e1 e2 (TypeBase TypeBool)
      *> return_rez (TypeBase TypeBool)
  | OpAssign ->
      find_expr_type e1 expr_tc >>= fun e ->
      compare_two_expr_type e1 e2 *> return_rez e
...
let tc_method_invoke e args expr_tc =
  expr_tc e >>= function
  | TCMethod m -> (
      tc_method_args m.method_params args expr_tc >>= fun _ ->
      match m.method_return with
      | TypeBase t ->
          let var_info =
            { var_type = TypeVar (TypeBase t); initialized = true }
          in
          return (TCLocalVar var_info)
      | TypeVoid ->
          fail
            (TCError (OtherError "Void methods cannot be used in expressions")))
  | TCField _ -> fail (TCError (OtherError "Cannot call a field as a method"))
  | TCLocalVar _ ->
      fail (TCError (OtherError "Cannot call a variable as a method"))

let check_initialized n =
  (* TODO: refactor to locals ?? *)
  read_local_el n >>= function
    ...
    | EId n ->
        ...
        <|> ( get_curr_class_name >>= fun class_name ->
              ...
              | None ->
                  fail
                    (TCError
                       (OtherError ("Variable not found: " ^ string_of_ident n)))
            )
    | EValue v ->
        let var_info =
          { var_type = TypeVar (value_to_type v); initialized = true }
        in
        return (TCLocalVar var_info)
...
let eq_type_with_expr t e =
  typecheck_expr_with_type e >>= fun e_t -> eq_type e_t t

let save_decl n ctx =
  read_local_el_opt n >>= function
...
let rec typecheck_stmt =
  ...
  let typecheck_return e_opt =
    ...
    | Some (TypeBase t), Some e ->
        ...
        <|> fail
              (TCError
                 (OtherError "Returned type does not match the function type"))
        )
        *> return ()
    | _ -> fail (TCError TypeMismatch)
  in
  ...
  let typecheck_if_state cond b s_opt tc_st =
    ...
    let typecheck_state = function Some st -> tc_st st | None -> return () in
  ...
  | SIf (e, b, s_opt) ->
      apply_local (typecheck_if_state e b s_opt typecheck_stmt)
...
let tc_member mem class_fields =
  ...
  let tc_class_method (mds, tp, id, pms, b) class_fields =
    ...
    if m.is_main then
      ...
      | [ MStatic ], Params [], TypeBase TypeInt
      | [ MStatic ], Params [], TypeVoid -> (
          tc_meth tp (Params []) b class_fields *> read_main_class >>= function
  ...
  | Method (mds, tp, id, pms, b) ->
      tc_class_method (mds, tp, id, pms, b) class_fields

let save_global id ctx =
  read_global_el_opt id >>= function
...
let typecheck prog =
  run (typecheck_obj prog) (IdMap.empty, IdMap.empty, None, None, None)
