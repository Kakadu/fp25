...
let chainl1 expr op =
  let rec pars e1 =
    lift2 (fun op1 e2 -> op1 e1 e2) op expr >>= pars <|> return e1
  in
  expr >>= pars

let chainr1 expr op =
  fix (fun x ->
      lift2 (fun op1 -> op1) (lift2 (fun e1 op2 -> op2 e1) expr op) x <|> expr)
...
let parse_int =
  take_while1 Char.is_digit >>= fun num ->
  return @@ ValInt (Int.of_string num) <|> fail "Not an int"
...
let parse_bool =
  choice
    [
      string "true" *> return (ValBool true);
      string "false" *> return (ValBool false);
    ]
  <|> fail "Not a bool"
...
(* Type words *)
let parse_type_word =
  take_while is_token_sym >>= function
  | "int" -> return TypeInt
...
let parse_id =
  take_while1 is_token_sym >>= fun str ->
  if String.is_empty str || in_reserved str || Char.is_digit str.[0] then
...
(* Variables && functions *)
let parse_var_type =
  choice ?failure_msg:(Some "Incorrect type") [ parse_base_type ] >>= fun x ->
  return (TypeVar x)

let parse_var =
  let parse_decl_id typ_ =
    skip_spaces *> parse_id >>| fun id -> Var (typ_, id)
  in
...
let parse_call_expr (arg : expr t) =
  parse_call_id >>= fun id -> parse_call_args id arg
...
let parse_assign =
  lift3 (fun id eq ex -> eq id ex) parse_id_expr ( ^=^ ) parse_ops
  <|> fail "Assign error"
...
let parse_if_else f_if_body =
  ...
  fix (fun ifls ->
      ...
      lift3
        ...
        parse_if_cond parse_body parse_else_body)
  <|> fail "If error"
...
let parse_break =
  skip_spaces *> string "break" *> return SBreak <|> fail "Break error"
...
let parse_field_sign =
  ...
  lift4
    ...
    (skip_spaces *> parse_id) (option None f_value)
  <* skip_semicolons1
...
let parse_method_member =
  lift2
    ...
    parse_method_sign parse_block

let parse_field_member =
  parse_field_sign >>| function
  | mds, tp, id, Some ex ->
      VarField (mds, tp, id, Some (EBinOp (OpAssign, EId id, ex)))
  | mds, tp, id, None -> VarField (mds, tp, id, None)

let parse_class_members =
  let member =
    choice ?failure_msg:(Some "Method error")
      [ parse_method_member; parse_field_member ]
  in
  braces @@ sep_by skip_spaces member

let parse_class =
  let class_id =
    skip_spaces *> string "class" *> skip_spaces *> parse_id
    <|> fail "Class sign error"
  in
  lift3
    (fun cl_mdf cl_id cl_mbs -> Class (cl_mdf, cl_id, cl_mbs))
    (skip_spaces *> parse_modifiers)
    class_id parse_class_members

let parse_prog : program t =
  parse_class <* skip_spaces >>| fun prog -> Program prog
...
let parse_option p str =
  match apply_parser p str with Ok x -> Some x | Error _ -> None
