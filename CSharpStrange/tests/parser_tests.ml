...
let%test "Parse parens" =
  apply_parser (parens parse_int) {|(1)|} = Ok (ValInt 1)

let%test "Parse braces" =
  apply_parser (braces parse_int) {|{1}|} = Ok (ValInt 1)

let%test "Parse brackets" =
  apply_parser (brackets parse_int) {|[1]|} = Ok (ValInt 1)

let%test "Parse one modifier 1" =
  apply_parser parse_modifiers {|static|} = Ok [ MStatic ]

let%test "Parse one modifier 2" =
  apply_parser parse_modifiers {|public|} = Ok [ MPublic ]
...
let%test "Parse add 2" =
  apply_parser parse_ops {|   a +   b|}
  = Ok (EBinOp (OpAdd, EId (Id "a"), EId (Id "b")))
...
let%test "Parse adds with mul 1" =
  ...
  = Ok
      (EBinOp
         ( OpAdd,
           EValue (ValInt 1),
           EBinOp (OpMul, EValue (ValInt 2), EValue (ValInt 3)) ))
...
let%test "Parse div with mod" =
  ...
  = Ok
      (EBinOp
         ( OpMod,
           EBinOp (OpDiv, EValue (ValInt 1), EValue (ValInt 2)),
           EValue (ValInt 3) ))
...
let%test "Parse id in expressions 1" =
  apply_parser parse_ops {| x|} = Ok (EId (Id "x"))

let%test "Parse id in expressions 2" =
  apply_parser parse_ops {|x + 1|}
  = Ok (EBinOp (OpAdd, EId (Id "x"), EValue (ValInt 1)))
...
let%test "Parse return 1" =
  apply_parser parse_return {|return 5|}
  = Ok (SReturn (Some (EValue (ValInt 5))))

let%test "Parse return 2" =
  apply_parser parse_return {|return|} = Ok (SReturn None)

let%test "Parse break" = apply_parser parse_break {|break|} = Ok SBreak

let%test "Parse continue" =
  apply_parser parse_continue {|continue|} = Ok SContinue

let%test "Parse empty block 1" =
  apply_parser parse_block {|{}|} = Ok (SBlock [])

let%test "Parse empty block 2" =
  apply_parser parse_block {|{;;;;}|} = Ok (SBlock [])
...
let%test "Parse block 2" =
  ...
  = Ok
      (SBlock
         [
           SDecl
             (Var (TypeVar (TypeBase TypeInt), Id "x"), Some (EValue (ValInt 6)));
           SExpr
             (EBinOp
                ( OpAssign,
                  EId (Id "x"),
...
let%test "Parse while" =
  ...
  = Ok
      (SBlock
         [
           SDecl
             (Var (TypeVar (TypeBase TypeInt), Id "x"), Some (EValue (ValInt 1)));
           SWhile
      ...
      (SBlock
         [
           SFor
             ( Some
                 (SDecl
                    ( Var (TypeVar (TypeBase TypeInt), Id "i"),
                      Some (EValue (ValInt 1)) )),
                 ...
                 (SIf
                    ( EBinOp (OpEqual, EId (Id "x"), EValue (ValInt 2)),
                      SBlock
                        [
                          SExpr
                            (EBinOp (OpAssign, EId (Id "x"), EValue (ValInt 2)));
                        ],
                      None )) );
         ])
...
let%test "Parse method 3" =
  apply_parser parse_method_member
