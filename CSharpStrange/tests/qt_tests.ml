...
let gen_ident =
  let open Gen in
  map (fun s -> if List.mem s reserved then Id "x" else Id s) string_small
;;
...
let rec gen_expr depth =
  ...
  then
    ...
    frequency
      ...
      ; 1, map2 (fun f args -> EFuncCall (f, Args args)) sub (list_size (1 -- 3) sub)
      ])
;;
    ...
    frequency
      ...
      ; ( 1
        , map3
            ...
            sub_stmt
            (option sub_stmt) )
      ; 1, map2 (fun cond body -> SWhile (cond, body)) (gen_expr depth) sub_stmt
...
let shrink_ident (Id s) =
  ...
  then return (Id (String.sub s 0 (String.length s - 1)))
  else empty
;;
...
let rec shrink_expr = function
  ...
  | EUnOp (op, e) -> Iter.return e <+> Iter.map (fun e' -> EUnOp (op, e')) (shrink_expr e)
...
let rec compare_expr_structure e1 e2 =
  ...
  | EUnOp (op1, e1), EUnOp (op2, e2) -> op1 = op2 && compare_expr_structure e1 e2
...
let prop_stmt_no_crash =
  Test.make
    ~name:"Statement parser does not crash"
    ~count:(test_count / 2)
    (stmt_arbitrary 3)
    (fun stmt ->
...
let prop_whitespace_handling =
  let gen = expr_arbitrary 3 in
  Test.make
    ~name:"Parser handles whitespace correctly"
    ~count:(test_count / 5)
    gen
    (fun expr ->
       let base_str = Ast.show_expr expr in
       let spaced_str = add_random_whitespace base_str in
       match apply_parser parse_ops base_str, apply_parser parse_ops spaced_str with
       | Ok expr1, Ok expr2 ->
         if compare_expr_structure expr1 expr2
         then true
         else (
...
let () =
  ...
  Printf.printf "\nRESULTS\n";
  if exit_code = 0
  then Printf.printf "All tests are executed!\n"
