Темы (предварительные) к экзамену по ФП  2026 для второго курса ПИ.
Вопросы-автоматы на оценку F обозначены ☭.

Формат:
1) ☭ Летучка на использование  CPS при написании функций. Для всех.
   (Проверка на минимальные навыки программирования.)
   Писать надо на OCaml, без ошибок, багов и со всеми скобками.
2) Летучка. Для тех, кто сделал допуск на оценку выше D. (Остальных спрошу по ходу дела.)
   Демонстрировать умение редуцировать лямбда-выражения во всех 4х стратегиях (CBN, CBV, NO, AO).
   (Выражение и стратегию выбирает экзаменатор.)
   Если разобрать заранее фибоначчи/факториал в каждой из стратегий, то будет гораздо легче.
...
4) Ответ принимающему. Вопросы из списка ниже без подготовки.
   Незнание расшифровок **аббревиатур** и прочих терминов будет награждаться оценкой F
...
1. ☭ Хвостовая рекурсия. Уметь объяснять, чем хвостовая лучше обычной обычной рекурсии примерах.
   ...
   * ☭ Понятие замыкания.
1. CPS. Преобразование функций из стандартного (direct) стиля в CPS
...
1. Лямбда-исчисление
   ...
   * ☭ Редексы. ☭ Нормальные формы. Стратегии вычисления лямбда-термов: CBN, CBV, call-by-need, NO, AO. Достоинства и недостатки
   * Семантика малого и большого шаго (хотя бы интуиция)
   * Упражнение: что будет если в составной стратегии NO везде писать CBN?
...
1. ZINC-машина. Примеры. Интерпретатор большого шага и малого.
1. PFDS. ☭ Понятие неизменяемых и устойчивых типов данных.
...
1. История Mars Climate Orbiter. Понятия: zero-cost abstraction.
   * Упражнение на реализацию сложения величин
