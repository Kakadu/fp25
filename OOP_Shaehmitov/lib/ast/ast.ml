...
type field_def = name * expr [@@deriving show { with_path = false }]

type class_def =
  { class_name : name
  ; class_params : pattern list
  ; parent_class : (name * expr list) option
...
let rec pp_expr fmt = function
  ...
  | FunExpr (params, body) ->
    let pp_param fmt p =
      match p with
      | PVar _ -> pp_pattern fmt p
      | PAny -> pp_pattern fmt p
      | PUnit -> pp_pattern fmt p
  ...
  | New (class_name, args) ->
    (match args with
     | [] -> Format.fprintf fmt "(new %s)" class_name
     | _ ->
       Format.fprintf
         fmt
         "(new %s %a)"
...
and pp_class_def
  fmt
  { class_name; class_params; parent_class; self_name; fields; methods }
  =
  if Base.List.is_empty class_params
  then Format.fprintf fmt "class %s =" class_name
  else
    Format.fprintf
      fmt
      "class %s %a ="
