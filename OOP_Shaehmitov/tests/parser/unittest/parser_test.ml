...
let%expect_test "method call chaining" =
  run_test "obj#method1#method2 true";
  [%expect
    {|
    (MethodCall ((MethodCall ((Var "obj"), "method1", [])), "method2",
       [(Const (CBool true))])) |}]
;;
    ...
    [(ClassDef
        { class_name = "point"; class_params = []; parent_class = None;
          ...
          [{ method_name = "get_x"; method_params = [];
             method_body = (MethodCall ((Var "this"), "x", [])) }
            ]
          })
      ] |}]
;;
    ...
    [(ClassDef
        { class_name = "colorPoint"; class_params = [];
          ...
          [{ method_name = "get_color"; method_params = [];
             method_body = (MethodCall ((Var "this"), "c", [])) }
            ]
          })
      ] |}]
;;
...
let%expect_test "if test in program" =
  run_program_parser "let f x = if(true)then(1)else 0 ;;";
  [%expect
    {|
    [(Value
        (NonRec, (PVar "f"),
         (FunExpr ([(PVar "x")],
            (If ((Const (CBool true)), (Const (CInt 1)), (Const (CInt 0))))))))
      ] |}]
;;

let%expect_test "tuple in tuple pattern" =
  run_program_parser "let (a, (b, c)) = (1, (2, 3)) ;;";
  [%expect
    {|
    [(Value
        (NonRec, (PTuple [(PVar "a"); (PTuple [(PVar "b"); (PVar "c")])]),
