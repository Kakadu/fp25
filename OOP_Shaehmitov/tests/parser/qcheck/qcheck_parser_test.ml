...
let rec shrink_expr = function
  ...
  | MethodCall (obj, _, _) ->
    let open Iter in
    return obj <+> (shrink_expr obj >|= fun obj' -> MethodCall (obj', "m", []))
;;
...
let gen_class_def size =
  ...
  let* class_params = list_size (int_range 0 2) (gen_pattern 0) in
  let* parent_class =
    option (pair gen_class_name (list_size (int_range 0 2) (gen_expr (size / 2))))
  in
  ...
  return { class_name; class_params; parent_class; self_name; fields; methods }
;;
